...
plt.title('GDP by Country', size=20)
plt.savefig("Bar_Chart_GDP.png")
plt.show()
...
plt.savefig("Bar_Chart_LEABY.png")
plt.show()
...
'''Life expectancy is almost the same in all the countries. Zimbabwe has a shorter life expectancy and extremely low GDP. US has overwhelming GDP compared to other countries and the life expectancy is close to 80 years of age.'''
...
plt.title('Life Expectancy Distribution', size=20)
plt.savefig("Violinplot_LEABY_Country.png")
plt.show()
...
'''Zimbabwe has had the most drastic change in life expectancy between all six countries.'''
...
plt.xticks(rotation=90)
plt.title('GDP over time', size=20)
plt.ylabel("GDP in Trillions of U.S. Dollars")
plt.savefig("Barplot_Country_GDP.png")
plt.show()
...
plt.title('Life expectancy over time', size=20)
plt.savefig("Barplot_Country_LEABY.png")
plt.show() 
...
'''Zimbabwe had a decline in the years 2000 to 2005 but had an exponential growth from the year 20006 to 2015. All other counrties experiencede linear growth in life expectancy.
Zimbabwe experience the least growth to GDP. China, however, has experienced exponential growth from year 2000 to 2016. China's life expectancy has also been a steady linear increase.

At first impression, it is safe to assume that there is a correlation between Life expectancy and GDP however, there is insuficient data to make an argument as to how much GDP life expectancy could contribute. 
USA, Chile, and Germany take lead in life expectancy however, the USA's GDP is significantly greater than all five countries combined. It isn't until the last few years that China has experience aggressive GDP growth.

Data on population, geography, and politics etc, can give us a clearer picture to understand '''
...
plt.savefig("FacetGrid_Scatter_LEABY_Year.png")
...
'''China's has the most progression in GDP over the years. Zimbabwe has the greatest increase in Life expectancy between the year 2000 to 2015.The data suggest that there are two countries that experience the most drastic growth, one in Life expectancy, and another one in GDP. 
The data from the graph is not surprising when comparing to the previous visualizations. I find the scatter plot more overwhelming to look at. Too many colors, and dots. ''' 
...
g3 = (g3.map(plt.plot, "Year", "LEABY", color='pink').add_legend())
plt.savefig("FaceGrit_LinePlot_LEABY.png")
...
'''In this graph it is evident to see that Zimbabwe has experience the most increase to life expectancy. The biggest change in data was between 2005 to 2010.

Mexico, Chile, and US have experienced the least change in the life expectancy. However, Chile has a higher life expectancy.

Politics, income, housing etc can influence the economic growth of a country.'''
...
g3 = sns.FacetGrid(df, col="Country", col_wrap=3, size=4)
g3 = (g3.map(plt.plot, "Year", "GDP", color="pink").add_legend())

plt.savefig("FaceGrid_LinePlot_GDP.png")
...
'''Highest GDP is US. Lowest GDP is Zimbabwe'''
...
'''Germany has the highest life expectancy. Zimbabwe has the lowest life expectancy'''
...
'''According to the World Economic Forum, there are atleast 10 ways in which Chinese economy has changes over the last decade contributing to its GDP. In this blog, I will mention only three. 

One: China has steadily become more self-sufficient and less reliant on revenue from export goods. China went from 35% of its GDP from exports in 2016, to 23% export revenue goods in 2014. Likewise, importation experience a decrease in the same years from 29% to 19%.
Two: Health and access to education are improving. A decrease in child mortality 2.19 in 2006 to 1.07 in 2015, and increase in enrollment to secondary school went from 68% in 2006 to 96% prior to 2013. Likewise, retention in education increase from 20% to 30% in the same years.
Three: Access to web. China went from approximately 10% of their population having access to web, to nearly everyone owning a mobile device. '''
...
Blog Report:
https://docs.google.com/document/d/1xQ8iSEGjwMsjN58A4Ea5IHR6WCYfocHkLKI-D7HrQcY/edit?usp=sharing
